...
import MessagesSection from '@/components/marketplace/MessagesSection';
...
export default function ProfilePage() {
  ...
  const userPosts = posts.filter((p) => p.isOwn);
  const recentPosts = userPosts.slice(0, 8);
  const saved = listings.filter((l) => savedListings.includes(l.id));
  const displayName = userProfile?.anonymousHandle ?? 'Anonymous Yapper';
      ...
      >
        <div className="mb-7 rounded-3xl bg-surface/60 px-5 pb-8 pt-8">
          <div className="flex flex-col items-center">
            ...
            >
              Verified · University of Exeter · {karma} karma
            </motion.p>
        ...
        >
          {[
            ...
            { icon: MessageSquare, value: userPosts.length, label: 'Yaps' },
                ...
                <div key={post.id} className="relative rounded-xl bg-background/80 px-3 transition-colors hover:bg-background">
                  ...
                  {post.isOwn && (
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 rounded-full p-2 transition-colors hover:bg-red-500/10"
